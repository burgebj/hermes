...
3. Store in Bitwarden Secrets Manager (preferred) or `~/.hermes/.env`:
   ```
...
Add those exports to your shell profile, or keep them in Bitwarden Secrets Manager / `~/.hermes/.env` so every session inherits them.
