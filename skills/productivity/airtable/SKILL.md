...
4. Store the token in Bitwarden Secrets Manager (preferred) or `~/.hermes/.env` (or via `hermes setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from the loaded environment (Bitwarden Secrets Manager or `~/.hermes/.env`) into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
