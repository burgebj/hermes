...
| **Rapid (MLX)** | `hermes model` → "Rapid (MLX)" (provider: `rapid-mlx`, aliases: `rapid`, `rapidmlx`, `rapid_mlx`; optional `RAPID_MLX_API_KEY`; Apple Silicon only — see [Rapid-MLX](https://github.com/raullenchai/Rapid-MLX)) |
...
### Rapid (MLX) — OpenAI-Compatible MLX Inference on Apple Silicon

[Rapid-MLX](https://github.com/raullenchai/Rapid-MLX) is an OpenAI- and Anthropic-compatible inference server for Apple Silicon, built on `mlx-lm` and `mlx-vlm`. Best for: Mac users who want the same wire format as cloud providers, with locally-hosted MLX models (Qwen, GLM, Hermes, Llama, Gemma, GPT-OSS, etc.) and a single `pipx`/`brew` install.
...
rapid-mlx serve qwen3.5-4b           # picks a curated alias, downloads on first run
...
hermes model
# Select "Rapid (MLX)"
# Press Enter to use http://127.0.0.1:8000/v1
# Pick one of the discovered models (rapid-mlx exposes the served alias via /v1/models)
# If rapid-mlx was started with --api-key, enter RAPID_MLX_API_KEY when prompted
```

**Tool calling:** Supported across all of rapid-mlx's bundled parsers (Hermes/Qwen/GLM/Harmony/MiniMax/etc.). The server detects the model family from the alias and selects the correct parser automatically.

**Multimodal:** rapid-mlx supports VLM-class aliases (Qwen3.5-VL, Gemma 4, Pixtral) via the same `/v1/chat/completions` endpoint. Hermes does not currently know which Rapid aliases are vision-capable (no `models.dev` catalog entry), so set `model.supports_vision: true` in your `~/.hermes/config.yaml` for the alias you've served before sending image inputs.

**Custom base URL:** Set `RAPID_MLX_BASE_URL` if the server is on another host or port. Hermes will use the env var when present, otherwise falls back to the default.
...
Supported providers: `openrouter`, `nous`, `novita`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `gemini`, `google-gemini-cli`, `qwen-oauth`, `huggingface`, `zai`, `kimi-coding`, `kimi-coding-cn`, `minimax`, `minimax-cn`, `minimax-oauth`, `deepseek`, `nvidia`, `xai`, `xai-oauth`, `ollama-cloud`, `bedrock`, `azure-foundry`, `opencode-zen`, `opencode-go`, `kilocode`, `xiaomi`, `arcee`, `gmi`, `stepfun`, `lmstudio`, `rapid-mlx`, `alibaba`, `alibaba-coding-plan`, `tencent-tokenhub`, `custom`.
