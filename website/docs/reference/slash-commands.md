...
| `/voice [on\|off\|tts\|narrate\|join\|channel\|leave\|status]` | Control spoken replies in chat. `narrate` sends text first, then long-form voice chunks; `join`/`channel`/`leave` manage Discord voice-channel mode. |
