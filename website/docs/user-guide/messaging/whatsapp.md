...
| **Bridge dies immediately with `code -9` under systemd (Ubuntu 24.04+)** | The bridge is getting SIGKILL'd by `systemd-oomd` because the unit's cgroup crosses the default memory-pressure threshold — usually due to the Node bridge's headless Chromium. Hermes v0.14.1+ ships the unit with `ManagedOOMMemoryPressure=auto` and `ManagedOOMSwap=auto` to opt out (see [#26997](https://github.com/NousResearch/hermes-agent/issues/26997)). If you installed before that release, run `hermes gateway install` to regenerate `hermes-gateway.service`, then `sudo systemctl daemon-reload && sudo systemctl restart hermes-gateway`. The symptom is "Bridge ready (status: connected)" → "Bridge started on port …" → "WhatsApp bridge process exited unexpectedly (code -9)" in the gateway log, while running the gateway in foreground works fine. |
