...
2. **От имени непривилегированного служебного пользователя** запустите обычный установщик. Он увидит отсутствие sudo, пропустит `--with-deps` и поставит Chromium в локальный cache Playwright:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/install.sh | bash
   ```
