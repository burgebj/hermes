...
В WSL2 по сути есть две изолированные среды: ваш Windows-host и Linux VM под управлением WSL. Большая часть путаницы возникает тогда, когда вы перестаёте понимать, на какой именно стороне находитесь в конкретный момент.
...
- использования **совместимого с OpenAI API-сервера** (который даёт `hermes gateway`, если `API_SERVER_ENABLED=true`) из инструмента на стороне Windows. См. [страницу API Server](/user-guide/features/api-server);
...
На **Windows 11 22H2+ с включённым mirrored mode** ничего делать не нужно. Процесс в WSL, который слушает `0.0.0.0:8080` (или даже `127.0.0.1:8080`), доступен из браузера Windows по `http://localhost:8080`. WSL автоматически пробрасывает привязку обратно на host.
...
cwd Hermes находится внутри файловой системы Linux, а `cmd.exe` на Windows не знает, что с ним делать. Запускайте Hermes из `/mnt/c/...` в этой сессии либо используйте wrapper, который перед вызовом Windows executable делает `cd` в путь, доступный Windows.
