...
Существует и лёгкий графический установщик — он удобен, если вам проще дважды щёлкнуть `.exe`, чем открывать PowerShell. Скачайте Hermes Desktop, запустите установщик, и при первом запуске GUI сам вызовет `install.ps1`, чтобы подготовить Python (через `uv`), Node, PortableGit и остальные зависимости в описанном ниже процессе первичной подготовки. После первого запуска настольное приложение и CLI, установленный через PowerShell, используют одну и ту же установку в `%LOCALAPPDATA%\hermes\hermes-agent` и общий каталог данных `%USERPROFILE%\.hermes` — можно свободно переключаться между GUI и CLI.
...
При первом запуске (и по требованию, когда Hermes обнаруживает, что чего-то не хватает) он запускает небольшой Python-bootstrapper — `hermes_cli/dep_ensure.py`. Он проверяет и при необходимости поднимает зависимости, не связанные с Python. На Windows к ним относятся:
...
Всё, кроме встроенной терминальной панели dashboard, работает в Windows нативно.
...
До PR #21561 на Windows нажатие `Ctrl-X Ctrl-E` или команда `/edit` тихо ничего не делали. prompt_toolkit использует жёстко заданный POSIX-список запасных редакторов (`/usr/bin/nano`, `/usr/bin/pico`, `/usr/bin/vi`, …), и на Windows он не срабатывает даже при полном Git for Windows.
...
`hermes gateway status` идемпотентен — вызывайте его хоть тысячу раз подряд, gateway случайно не завершится. (До PR #21561 он этого тихо и делал из-за `os.kill(pid, 0)`, который на уровне C конфликтовал с `CTRL_C_EVENT` — если интересна история, см. раздел про внутренности управления процессами ниже.)

### Почему не Windows Service?

Службы требуют админских прав на установку и привязывают жизненный цикл gateway к загрузке машины, а не к входу пользователя. Типичный пользователь Hermes ожидает другого: вошёл в систему → gateway доступен, вышел из системы → gateway завершился. Scheduled Tasks делают именно это и не требуют повышения прав. Если вам действительно нужна служба, можно использовать `nssm` или `sc create` вручную — но, скорее всего, в этом нет необходимости.
...
Проверьте `hermes gateway status` — он объединяет запись schtasks, ярлык в Startup (если он использовался) и живой PID. Если schtasks зарегистрирован, но gateway не стартует, групповая политика может блокировать триггеры `ONLOGON`. Выполните `schtasks /Query /TN HermesGateway /V /FO LIST`, чтобы увидеть причину сбоя, или откатитесь к сценарию со Startup folder, удалив и установив Hermes заново с `HERMES_GATEWAY_FORCE_STARTUP=1`.
...
**Gateway не может отправить фото в Telegram — "`BadRequest: payload contains invalid characters`".**
