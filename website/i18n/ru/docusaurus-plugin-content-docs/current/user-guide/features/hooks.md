...
Объявляйте shell-script хуки в `cli-config.yaml`, и Hermes будет запускать их как subprocess при каждом срабатывании соответствующего события плагин-хука — и в CLI, и в gateway-сеансах. Писать Python-плагин не нужно.

Используйте shell-хуки, когда нужен простой одnofайловый скрипт (Bash, Python, любой язык с shebang), чтобы:
