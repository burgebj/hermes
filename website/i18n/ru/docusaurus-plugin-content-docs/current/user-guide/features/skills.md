...
- **Наборы не сбрасывают кеш подсказки.** При вызове они создают свежее пользовательское сообщение так же, как `/<skill-name>`, без изменения системной инструкции.
...
Просматривайте, ищите, устанавливайте и управляйте навыками из онлайн-реестров, `skills.sh`, прямых well-known конечных точек и официальных optional skills.

### Частые команды

```bash
hermes skills browse                              # Просмотреть все навыки хаба (сначала official)
hermes skills browse --source official            # Просмотреть только официальные optional skills
...
|--------|---------|------------|
| `official` | `official/security/1password` | Optional skills, поставляемые вместе с Hermes. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Доступно через `hermes skills search <query> --source skills-sh`. Hermes умеет разбирать alias-слуги, если slug в skills.sh отличается от имени папки в репозитории. |
...
#### 1. Официальные optional skills (`official`)
...
Это публичный каталог навыков от Vercel. Hermes может искать по нему напрямую, просматривать страницы навыков, разбирать alias-slugs и устанавливать навыки из исходного репозитория.
...
Это URL-based discovery через сайты, которые публикуют `/.well-known/skills/index.json`. Это не единый центральный хаб, а веб-конвенция для обнаружения навыков.

- Живой пример endpoint'а: [Mintlify docs skills index](https://mintlify.com/docs/.well-known/skills/index.json)
...
- Официальные optional skills (`official/...`) считаются trust builtin и не показывают панель предупреждения о стороннем источнике.
...
Официальные optional skills по-прежнему используют идентификаторы вроде `official/security/1password` и `official/migration/openclaw-migration`.
