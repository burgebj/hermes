...
| Навык | Описание |
|-------|-------------|
| [**3-statement-model**](/docs/user-guide/skills/optional/finance/finance-3-statement-model) | Строит полноценно связанные финансовые модели формата 3-statement (IS, BS, CF) в Excel с графиками оборотного капитала, D&A roll-forward, debt schedule и связками, которые сводят cash и retained earnings. Пара к `excel-author`. |
| [**comps-analysis**](/docs/user-guide/skills/optional/finance/finance-comps-analysis) | Строит сравнительный анализ компаний-аналогов в Excel: операционные метрики, мультипликаторы оценки и статистический бенчмарк по группе аналогов. Пара к `excel-author`. Подходит для публичной оценки компаний, ценообразования IPO, отраслевого сравнения и поиска выбросов. |
| [**dcf-model**](/docs/user-guide/skills/optional/finance/finance-dcf-model) | Строит DCF-модели институционального уровня в Excel: прогнозы выручки, FCF build, WACC, terminal value, сценарии Bear/Base/Bull, таблицы чувствительности 5x5. Пара к `excel-author`. Подходит для фундаментальной оценки акций по внутренней стоимости. |
| [**excel-author**](/docs/user-guide/skills/optional/finance/finance-excel-author) | Собирает аудируемые Excel-книги без GUI через `openpyxl` — цветовые соглашения blue/black/green, формулы вместо хардкода, именованные диапазоны, проверка баланса и таблицы чувствительности. Подходит для финансовых моделей, проверочных отчётов и сверок. |
| [**lbo-model**](/docs/user-guide/skills/optional/finance/finance-lbo-model) | Строит LBO-модели в Excel: sources & uses, debt schedule, cash sweep, exit multiple, чувствительность IRR/MOIC. Пара к `excel-author`. Подходит для отбора private equity-сделок, оценки sponsor-case и иллюстративных LBO в презентациях. |
| [**merger-model**](/docs/user-guide/skills/optional/finance/finance-merger-model) | Строит модели аккреции и размывания (merger) в Excel: pro forma P&L, synergy, структура финансирования, влияние на EPS. Пара к `excel-author`. Подходит для M&A-презентаций, материалов для совета директоров и оценки сделки. |
| [**pptx-author**](/docs/user-guide/skills/optional/finance/finance-pptx-author) | Собирает презентации PowerPoint без GUI через `python-pptx`. Пара к `excel-author` для презентаций, где каждая цифра должна быть привязана к ячейке рабочей книги. Подходит для pitch deck, IC memo и заметок по отчётности. |
