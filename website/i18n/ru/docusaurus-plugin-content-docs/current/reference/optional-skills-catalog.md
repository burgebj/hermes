...
| Навык | Описание |
|-------|-------------|
| [**3-statement-model**](/docs/user-guide/skills/optional/finance/finance-3-statement-model) | Строит полноценно связные 3-statement модели (IS, BS, CF) в Excel с графиками оборотного капитала, D&A roll-forward, debt schedule и связками, которые сводят cash и retained earnings. Пара к `excel-author`. |
| [**comps-analysis**](/docs/user-guide/skills/optional/finance/finance-comps-analysis) | Строит comparable company analysis в Excel: операционные метрики, мультипликаторы оценки, статистический бенчмарк по peer set. Пара к `excel-author`. Подходит для публичной оценки компаний, IPO pricing, отраслевого бенчмарка и поиска выбросов. |
| [**dcf-model**](/docs/user-guide/skills/optional/finance/finance-dcf-model) | Строит институционального уровня DCF-модели в Excel: прогнозы выручки, FCF build, WACC, terminal value, сценарии Bear/Base/Bull, таблицы чувствительности 5x5. Пара к `excel-author`. Подходит для intrinsic-value equity analysis. |
...
| [**lbo-model**](/docs/user-guide/skills/optional/finance/finance-lbo-model) | Строит LBO-модели в Excel: sources & uses, debt schedule, cash sweep, exit multiple, чувствительность IRR/MOIC. Пара к `excel-author`. Подходит для PE screening, sponsor-case valuation и иллюстративных LBO в презентациях. |
| [**merger-model**](/docs/user-guide/skills/optional/finance/finance-merger-model) | Строит модели аккреции/размывания (merger) в Excel: pro forma P&L, synergy, структура финансирования, влияние на EPS. Пара к `excel-author`. Подходит для M&A-презентаций, материалов для совета и оценки сделки. |
| [**pptx-author**](/docs/user-guide/skills/optional/finance/finance-pptx-author) | Собирает презентации PowerPoint без GUI через `python-pptx`. Пара к `excel-author` для deck'ов, где каждая цифра должна отсылаться к ячейке workbook. Подходит для pitch deck, IC memo и earnings notes. |
