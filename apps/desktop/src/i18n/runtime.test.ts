...
describe('desktop i18n runtime translator', () => {
  ...
  it('translates migrated overlap keys for newly supported locales', () => {
    ...
    setRuntimeI18nLocale('es')
    expect(translateNow('common.save')).toBe('Guardar')
    expect(translateNow('language.label')).toBe('Idioma')
  })

  it('passes arguments to Spanish function translations', () => {
    setRuntimeI18nLocale('es')

    expect(translateNow('notifications.updateReadyMessage', 2)).toBe('2 cambios nuevos disponibles.')
  })
  ...
  it('translates settings copy for newly supported locales', () => {
    ...
    setRuntimeI18nLocale('es')
    expect(translateNow('settings.appearance.title')).toBe('Apariencia')
    expect(translateNow('settings.nav.providerApiKeys')).toBe('Claves API')
  })
