...
- Image editing / image-to-image via ``/v1/images/edits``
...
def _extract_and_cache_image(
    ...
    *,
    ...
    to the bare URL when the download fails — xAI's ``imgen.x.ai`` URLs expire
    fast, so a cache miss must not become a hard tool error); anything else is
    an ``empty_response``.
    """
    ...
    def edit(
        ...
        **kwargs: Any,
        ...
        payload: Dict[str, Any] = {
            "model": model_id,
            "prompt": prompt,
            # Output aspect mirrors the generation surface; cheap to drop
            # should the edits endpoint ever reject it.
            "aspect_ratio": xai_ar,
