...
class SimplexAdapter(BasePlatformAdapter):
    ...
    async def _health_monitor(self) -> None:
        """Log when the WebSocket has been idle. ping/pong handles real reconnects."""
        while self._running:
            ...
            if elapsed > HEALTH_CHECK_STALE_THRESHOLD:
                logger.debug(
                    "SimpleX: WS idle for %.0fs (no user messages, connection "
                    "healthy via ping/pong)",
                    elapsed,
                )
    ...
    async def _handle_event(self, event: dict) -> None:
        ...
        # simplex-chat WebSocket API sends JSON with a ``resp`` key
        # containing the event type and a ``corrId`` for correlated
        # request/response. Some events also use a top-level ``type`` field.
        resp = event.get("resp", {}) or {}
...
def interactive_setup() -> None:
    ...
    _prompt(
        ...
        "Allowed contact IDs (comma-separated; blank=skip)",
    )
    _prompt(
