...
def test_message_timestamp_context_rendering_is_opt_in():
    from gateway.run import _build_gateway_agent_history

    ts = _epoch(2026, 4, 28, 13, 40, 53)
    history = [{"role": "user", "content": "hello", "timestamp": ts}]

    default_history, _ = _build_gateway_agent_history(history)
    enabled_history, _ = _build_gateway_agent_history(
        history,
        inject_message_timestamps=True,
    )

    assert default_history[0]["content"] == "hello"
    assert enabled_history[0]["content"].endswith(" hello")
    assert "2026-04-28" in enabled_history[0]["content"]


def test_message_timestamp_config_defaults_off_and_opt_in_enabled():
    from gateway.run import _message_timestamps_enabled

    assert _message_timestamps_enabled({}) is False
    assert _message_timestamps_enabled({"gateway": {"message_timestamps": {"enabled": True}}}) is True
