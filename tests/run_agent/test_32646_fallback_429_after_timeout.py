...
Reproduces the (timeout × N → recover → 429 → no fallback) sequence
...
Scenario:
  ...
  4. The fix resets ``_fallback_index`` / ``_fallback_activated`` /
     ``has_retried_429`` after transport recovery so the post-recovery
...
class TestFallbackChainResetOnTransportRecovery:
    ...
    def test_has_retried_429_resets_to_false_after_recovery(self):
        """``has_retried_429`` is a local variable in the conversation
        loop, so this test asserts the documented semantics: a fresh
        attempt cycle after primary recovery should start with the
        credential-pool retry flag cleared so a single-credential pool
        gets the cheap retry-same-credential pass before rotation.
        """
        # Local-variable semantics: simulate the conversation loop body.
        has_retried_429 = True  # set by a pre-recovery 429 path
        primary_recovery_attempted = False

        # The fix block:
        recovered = True  # _try_recover_primary_transport() returned True
        if recovered and not primary_recovery_attempted:
            primary_recovery_attempted = True
            has_retried_429 = False  # ← the documented reset
