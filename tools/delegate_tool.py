...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        f"delegation.max_concurrent_children in config.yaml). "
        f"All run in parallel and results are returned together. Batch routing "
        f"is per-task only: put model/provider/reasoning_effort inside each "
        f"tasks[] item. {nesting_clause}\n\n"
