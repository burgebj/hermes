...
def _build_child_system_prompt(
    ...
    *,
    ...
    parts.append(
        ...
        "Before claiming a task is done, you MUST run the verification command "
        "yourself and quote its output. Saying \"tests pass\" without showing "
        "the runner's exit line is forbidden. Saying \"CI is green\" without "
        "quoting `gh run view` output is forbidden. Saying \"build succeeded\" "
        "without showing the build command's last line is forbidden.\n"
        "If a process you spawned is still running (dev server, watch mode, "
        "long-lived test runner), the task is NOT done — kill it, wait for it, "
        "or explicitly mark the task blocked. Never report success while a "
        "foreground process is still alive in your shell.\n"
        "If your last tool call timed out, hung, or produced no exit line, "
        "treat the outcome as UNKNOWN, not success. Re-run or report blocked.\n\n"
