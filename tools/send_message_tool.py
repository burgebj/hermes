...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    # --- Email: native MIME attachment support via SMTP ---
...
async def _send_email_with_media(extra, chat_id, message, media_files=None):
    """Send via SMTP with MEDIA files as MIME attachments."""
    import smtplib
    ...
    smtp_host = extra.get("smtp_host") or os.getenv("EMAIL_SMTP_HOST", "")
    try:
        smtp_port = int(extra.get("smtp_port") or os.getenv("EMAIL_SMTP_PORT", "587"))
    except (ValueError, TypeError):
        smtp_port = 587
    ...
    for media_path, _is_voice in media_files or []:
        if os.path.exists(media_path):
            ...
            skipped_paths.append(media_path)
            logger.warning("send_message email: skipping missing MEDIA file %s", media_path)

    if not valid_paths and not message.strip():
    ...
    try:
        ...
        body = message if message.strip() else "See attached file(s)."
