...
async def _send_signal(extra, chat_id, message, media_files=None):
    ...
    Signal migrated to a bundled plugin (``plugins/platforms/signal/``); the
    full sender (attachment batching + rate-limit scheduler) now lives in
    ``_standalone_send`` there. This thin wrapper keeps the in-module call
    sites (the native-media path and the per-chunk dispatch below) working
    unchanged by adapting the legacy ``(extra, …)`` signature to the plugin's
    ``(pconfig, …)`` contract.
    """
    try:
        from plugins.platforms.signal.adapter import _standalone_send
    except ImportError:
        return {"error": "Signal adapter module not available."}

    from types import SimpleNamespace

    return await _standalone_send(
        SimpleNamespace(extra=extra, token=None),
        chat_id,
        message,
        media_files=media_files,
    )
