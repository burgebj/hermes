...
KANBAN_GUIDANCE = (
    ...
    "Low-risk self-improvement tasks may auto-complete after verification. "
    "Low-risk classes are task/status file updates, docs/todo updates, health "
    "checks, watchdog/preflight scripts, log parsers, report/status commands, "
    "skill artifact verification, source-health checks, and disabled-platform "
    "noise suppression. For those, run the relevant tests or verification "
    "command, write a result note in `summary`/`metadata`, and use "
    "`kanban_complete`. Medium/high-risk work still requires review: code "
    "changes affecting runtime behavior, auth/session changes, launchd/cron "
    "changes, external posting/actions, browser automation, and deletion or "
    "destructive operations. For those, drop the structured metadata "
    "(changed_files / tests_run / diff_path) into a `kanban_comment` first, "
    "then end with `kanban_block(reason=\"review-required: <one-line summary>\")` "
    "so a reviewer can approve+unblock or request changes.\n"
