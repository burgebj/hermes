...
KANBAN_GUIDANCE = (
    ...
    "## Lifecycle\n"
    "\n"
    "1. **Orient + announce.** Call `kanban_show()` first (no args — it "
    "defaults to your task). The response includes title, body, parent-task "
    "handoffs (summary + metadata), any prior attempts on this task if "
    "you're a retry, the full comment thread, and a pre-formatted "
    "`worker_context` you can treat as ground truth. IMMEDIATELY after "
    "orient, post a starting comment via `kanban_comment(body=...)` — one "
    "sentence naming what you're about to do. Operators learn that you're "
    "alive from this comment; silent workers look hung.\n"
    ...
    "5. **Push, PR, then complete.** For any task where you committed code: "
    "before calling `kanban_complete`, `git push -u origin <branch>` and "
    "`gh pr create --title \"<concise>\" --body \"Closes "
    "t_$HERMES_KANBAN_TASK. <summary>\"`. Capture the PR url. Then call "
    "`kanban_complete(summary=..., metadata=...)`. `summary` is 1–3 "
    "human-readable sentences naming concrete artifacts AND the PR url. "
    "`metadata` is machine-readable facts including `pr_url`, `pr_number`, "
    "`branch`, `commit`, `changed_files`, `tests_passed`. Downstream "
    ...
    "eyes on it.\n"
    "Best-effort: also `openclaw message send --channel discord --account "
    "default --text \"🦞 ${HERMES_KANBAN_TASK}: done. PR: <url>\" "
    "2>/dev/null || true` so the Clawta persona narrates the dispatch live "
    "in the operator's Discord channel. Skip on failure — kanban is "
    "durable, Discord is the real-time mouth.\n"
