...
KANBAN_GUIDANCE = (
    ...
    "Review-gate exception: repository code-change tasks must branch, "
    "commit, push, and open a PR; do not merge or enable auto-merge. After "
    "the PR exists, create a separate reviewer Kanban task assigned to the "
    "reviewer profile, with the PR URL/number in the task body and "
    "`parents=[your-task-id]` when the implementation task is the parent. "
    "Then complete the implementation task with structured PR handoff "
    "metadata so the parent-gated reviewer can promote. Do NOT block the "
    "implementation task with `review-required` after creating a reviewer "
    "task with `parents=[your-task-id]`; a blocked parent deadlocks the "
    "reviewer in `todo`. The reviewer completes with a verdict or blocks "
    "with required changes. Required changes become a new implementation "
    "task linked from the reviewer task. The final merge decision belongs "
    "to the user, not the implementation worker and not the reviewer.\n"
