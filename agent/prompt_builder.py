...
COMPUTER_USE_GUIDANCE = (
    ...
    "You have Codex-style `computer_use_*` tools that drive the macOS desktop "
    "in the BACKGROUND — your actions do not steal the user's cursor, keyboard "
    ...
    "2. Act by app + element index: `computer_use_click(app='Safari', element=14)`. "
    "This is dramatically more reliable than pixel coordinates for any model. "
    "Use raw coordinates only as a last resort.\n"
    "3. Mutating tools should include `app`. For text input, use "
    "`computer_use_type_text(app='Safari', text='...')`. For key combos use "
    "`computer_use_press_key(app='Safari', key='cmd+s')`. For scrolling use "
    "`computer_use_scroll(app='Safari', direction='down', pages=1)`.\n"
    "4. After any state-changing action, call `computer_use_get_app_state` "
    "again to verify. You can pass `capture_after=true` to get the follow-up "
    "screenshot in one round-trip.\n\n"
    ...
    "## Daemon\n"
    "If the Computer Use driver is unavailable or you need a clean restart, "
    "call `computer_use_daemon(action='status'|'start'|'stop')` instead of "
    "shelling out via terminal.\n\n"
    "## Safety\n"
