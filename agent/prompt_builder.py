...
PLATFORM_HINTS = {
    ...
    "feishu": (
        ...
        "Feishu IM renders ONLY a SUBSET of Markdown — choose formatting carefully:\n"
        "  ✓ WORKS: **bold**, *italic*, `inline code`, ```fenced code blocks```, "
        "[links](url), bullet lists (- / *), numbered lists (1.).\n"
        "  ✗ DOES NOT RENDER: Markdown TABLES (| a | b |) and ATX headings "
        "(# Title, ## Subtitle, ###...) — these will appear as raw source text. "
        "Block quotes (>) and horizontal rules (---) also do not render.\n"
        "  → For section headings, use a **bold line** instead of #/##.\n"
        "  → For tabular data, use a bullet list of key-value pairs (e.g. "
        "'- **方案 A**: 描述...') instead of a Markdown table. The Hermes Feishu "
        "adapter MAY auto-convert tables to interactive cards when supported, but "
        "key-value bullet lists are always safe.\n"
        "You can send media files natively: include MEDIA:/absolute/path/to/file "
