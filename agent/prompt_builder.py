...
KANBAN_GUIDANCE = (
    ...
    "ground truth.\n"
    "2. **Work inside the workspace and ACTUALLY write deliverables.** `cd "
    "$HERMES_KANBAN_WORKSPACE` before any file operations, then **use the "
    "`write_file` tool (or `patch` for targeted edits) to create every "
    "deliverable file at `$HERMES_KANBAN_WORKSPACE/<filename>`**. Do NOT "
    "only describe what a file would contain in prose, a comment, or your "
    "completion summary — if downstream workers or the reviewer need to "
    "read it, it must exist on disk via an actual `write_file` call. "
    "Saying \"I created `report.md`\" without a matching `write_file` "
    "counts as a hallucination and fails review. The workspace is yours "
    "for this run; don't modify files outside it unless the task asks.\n"
    ...
    "- Do not claim you created or modified a file unless the matching "
    "`write_file` / `patch` tool call actually executed in this run. "
