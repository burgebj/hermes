...
KANBAN_GUIDANCE = (
    ...
    "You have ONE task from `~/.hermes/kanban.db`; task id is "
    "`$HERMES_KANBAN_TASK`, workspace is `$HERMES_KANBAN_WORKSPACE`. "
    "Use `kanban_*` tools, not `hermes kanban` shell commands.\n\n"
    "1. Orient first: call `kanban_show()` and treat `worker_context` as ground truth. "
    "Extract acceptance gates before working: explicit requirements plus tests/build/lint, "
    "live checks, benchmark/release/doc artifacts as applicable.\n"
    "2. Work only inside `$HERMES_KANBAN_WORKSPACE` unless explicitly told otherwise. "
    "Heartbeat during long operations.\n"
    "3. Block only for genuine external ambiguity/blockers: credentials, paid/cloud access, "
    "destructive irreversible actions, missing human decisions, or explicit approval requirements. "
    "If a requirement cannot be met after a serious attempt, add a `SCOPE_CHANGE_REQUEST` "
    "comment with requirement_ref, requested_change, reason, attempted_evidence, impact, "
    "options; then `kanban_block(reason=...)`.\n"
    "4. Complete with evidence: `kanban_complete(summary=..., metadata=...)` must include "
    "or be preceded by a gate scorecard: gate, command/check, exit code/artifact path, "
    "verdict, rationale. Never store secrets/tokens/raw PII in durable summaries, comments, "
    "events, or metadata. Benchmark/performance claims need real artifacts.\n"
    "5. For substantive code/product work in autonomous/yolo Office flows, do not block only "
    "because review is needed. Complete the worker handoff with metadata such as "
    "`handoff_for_supervisor_review: true`, `human_review_required: false` unless explicit "
    "human approval is required, changed files, tests run, diff/artifact paths, and known risks. "
    "The supervisor/reviewer/QA pipeline validates evidence after handoff.\n"
    "6. If follow-up work appears, create a child task for the right specialist via "
    "`kanban_create(..., parents=[your-task-id])`; do not scope-creep or assign it to yourself.\n\n"
    "Orchestrators decompose with `kanban_create` and then complete their routing task; "
    "do not implement the work. Do not complete unfinished work, and do not use "
    "`delegate_task` as a persistent board substitute."
)
