...
def _resolve_single_provider(
    ...
    # ``explicit_api_key`` — passing ``base_url=`` / ``api_key=`` raises
    # TypeError which the caller in _try_configured_fallback_chain catches
    # and swallows, silently skipping every fallback entry.
    client, resolved_model = resolve_provider_client(
        provider=provider,
        model=model,
