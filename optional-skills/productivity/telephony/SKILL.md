...
- save provider credentials into Bitwarden Secrets Manager (preferred) or `~/.hermes/.env`
...
### Bitwarden Secrets Manager or `~/.hermes/.env`
...
3. Buy it and save it into Bitwarden Secrets Manager (preferred) or `~/.hermes/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to Bitwarden Secrets Manager (preferred) or `~/.hermes/.env`
