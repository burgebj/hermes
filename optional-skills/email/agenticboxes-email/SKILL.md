...
- **Direct API** — invoke `terminal` to call `https://api.agenticboxes.email/api/v1/...`
  with the bearer token. This pattern works on any platform that has
  `curl`-equivalent (every supported platform does).
