...
_PLATFORMS = [
    ...
    },
    {
        ...
        "emoji": "🪝",
        "setup_instructions": [
            "Svix consumes webhook events via polling — no public ingress required.",
            "",
            "1. Install the Svix CLI:",
            "npm i -g svix-cli or check the docs for other options https://docs.svix.com/tutorials/cli",
            "2. Authenticate the CLI: `svix login`",
            "3. Create an Ingest Source in the Svix dashboard or via CLI:",
            "     svix ingest source list",
            "     svix ingest source create '{\"name\": \"my-source\", \"type\": \"genericWebhook\"}'",
            "   Copy the polling URL — it looks like",
            "   https://api.svix.com/api/v1/app/app_xxx/poller/poll_yyy/",
            "4. Copy the endpoint-scoped token (sk_endp_…) from the dashboard or:",
            "     svix ingest source get <source_id>",
            "5. Add the URL and token to each route under platforms.svix.extra.routes",
            "   in config.yaml (auth_token_env: MY_ENV_VAR).",
        ],
        "vars": [
            {"name": "SVIX_POLL_INTERVAL", "prompt": "Seconds between polls when caught up (default: 5)", "password": False,
             "help": "Adapter sleeps this long after Svix says there are no new events."},
        ],
    },
    {
        "key": "yuanbao",
